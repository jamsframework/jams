...
title="ABCModel",
    ...
    public void run() {
       ...
       // for (int i=0;i<=20000000;i++){ //cluster
        for (int i=0;i<=1000000;i++){ //laptop
         sqrt = Math.sqrt(Math.sqrt(a*i-i)*precip+Math.sqrt(b*i));
      //   sqrt1 = Math.sqrt(Math.sqrt(a*i-i)*precip+Math.sqrt(b*i)*i);
         if (i==20000000){
             Thread.currentThread().sleep(1);
         //getModel().getRuntime().println("ABCModel executed. lastvalue");
         }        
       }
        }
        catch (Exception e){}
