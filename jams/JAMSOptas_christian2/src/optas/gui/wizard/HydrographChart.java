...
public class HydrographChart {
    ...
    TimeSerie<Double> hydrograph;
    TimeSerie<Double> groundwater;
    ...
    DefaultTimeFilter selectedTimeFilter = null;
    ...
    public void setHydrograph(TimeSerie<Double> hydrograph) {
    ...
    public void addTimeFilter(DefaultTimeFilter filter){
    ...
    public void addTimeFilters(TimeFilterCollection timeFilters){
        ...
        for (DefaultTimeFilter f : timeFilters.get()){
            this.filters.add(f);
        }
        update();
    }
    ...
    public void setSelectedTimeFilter(DefaultTimeFilter f){  
    ...
    public void update(){       
        Runnable r = new Runnable() {
            ...
            public void run() {
                ...
                long n = hydrograph.getTemporalDomain().getNumberOfTimesteps();
                ...
                if (seriesHydrograph == null){
                    ...
                    for (int i = 0; i < n; i++) {
                        if (hydrograph.getValue((int) i) == JAMS.getMissingDataValue()) {
                            if (hydrograph.getTemporalDomain().getTimeUnit() == Calendar.MONTH) {
                                seriesHydrograph.add(new Month(hydrograph.getTime((int) i), GMT_TZ), Double.NaN);
                            } else if (hydrograph.getTemporalDomain().getTimeUnit() == Calendar.DAY_OF_YEAR) {
                                seriesHydrograph.add(new Day(hydrograph.getTime((int) i), GMT_TZ), Double.NaN);
                            } else if (hydrograph.getTemporalDomain().getTimeUnit() == Calendar.HOUR_OF_DAY) {
