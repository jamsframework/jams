...
import optas.data.time.TimeFilterFactory.BaseFlowTimeFilter;
import optas.data.time.TimeFilterFactory.EventFilter;
import optas.data.time.TimeFilterFactory.MonthlyTimeFilter;
import optas.data.time.TimeFilterFactory.RangeTimeFilter;
import optas.data.time.TimeFilterFactory.YearlyTimeFilter;
...
public class TimeFilterDialog extends JDialog{
    ...
    public TimeFilterDialog(ComparableTimeSerie<Double> timeserie){
        this.timeserie = timeserie;
        init(timeserie,null); //1,2,3
    }

    static public boolean isApplicable(DataCollection dc){
        return !dc.getDataSetNames(TimeSerie.class).isEmpty();
    }
    ...
    private JPanel constructYearTimeFilterPanel(YearlyTimeFilter filter) {
        ...
        TimeInterval t = timeserie.getTemporalDomain();
    ...
    private DefaultTimeFilter constructHydroEventFilter(){
        int windowSize;
    ...
    private DefaultTimeFilter constructRangeFilter(){
        ...
        I.setValue(timeserie.getTemporalDomain().getValue());
        I.setStart(cStart);
        I.setEnd(cEnd);
    ...
    private JPanel constructRangeFilterPanel(TimeSerie m, RangeTimeFilter f){
        ...
        if (m != null && f == null){
            rangeFilterStartDateInput.setValue(m.getTemporalDomain().getStart());
            rangeFilterEndDateInput.setValue(m.getTemporalDomain().getEnd());
        }
        if (f != null){
    ...
    private JPanel constructHydroEventFilterPanel(TimeSerie m, EventFilter filter) {
        ...
        switch (m.getTemporalDomain().getTimeUnit()){
    ...
    private DefaultTimeFilter constructBaseFlowFilter(){
    ...
    private JPanel constructBaseFlowTimeFilterPanel(ComparableTimeSerie<Double> m, BaseFlowTimeFilter filter) {
        JPanel filterPanel = new JPanel(new BorderLayout());
    ...
    public void init(ComparableTimeSerie<Double> serie, DefaultTimeFilter filter){
        this.filter = filter;
    ...
    public DefaultTimeFilter getFilter(){
        return this.filter;
    }
    @Override
