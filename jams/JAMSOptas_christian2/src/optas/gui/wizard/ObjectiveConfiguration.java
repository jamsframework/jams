...
public class ObjectiveConfiguration extends JPanel {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (f.exists()) {
                    try {
                        ...
                        Logger.getLogger(ObjectiveConfiguration.this.getClass().getName()).log(Level.SEVERE, t.toString(), t);
                    }
                }
            }
        });
        ...
        filterList.getTable().getSelectionModel().addListSelectionListener(new ListSelectionListener() {
            ...
            public void valueChanged(ListSelectionEvent e) {
                if (!e.getValueIsAdjusting()) {
                    ...
                    if (index != -1) {
                        DefaultTimeFilter f = filterList.getTimeFilters().get(index);
            ...
            public void chartMouseClicked(ChartMouseEvent cme) {
                ...
                if (!hydroChart.filters.combine().isFiltered(time)) {
                    for (int i = 0; i < hydroChart.filters.size(); i++) {
                        DefaultTimeFilter f = hydroChart.filters.get(i);
    ...
    private void loadTimeseries(File f) throws IOException {
        J2KTimeSerieReader tsr = new J2KTimeSerieReader(f);;
        ComparableTimeSerie<Double> ts = tsr.getData().getColumn(0);

        modelTimeIntervalInput.setValue(ts.getTemporalDomain().getValue());
