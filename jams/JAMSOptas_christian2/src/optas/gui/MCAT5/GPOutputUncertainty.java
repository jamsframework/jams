...
public class GPOutputUncertainty extends MCAT5Plot {
    ...
    public GPOutputUncertainty() {
        this.addRequest(new SimpleRequest(JAMS.i18n("SIMULATED_TIMESERIE"), MeasuredTimeSerie.class, 0, 10));
        this.addRequest(new SimpleRequest(JAMS.i18n("OBSERVED_TIMESERIE"), MeasuredTimeSerie.class, 0, 1));
    ...
    private String provideBasicInformation(double nlZ, 
            TimeSerie<Double> ts_obs, 
            TimeSerie<Double> ts_sim, 
            TimeSerie<Double> ts_lb, 
            TimeSerie<Double> ts_ub, 
            TimeSerie<Double> ts_exp, 
            int trainSamples, int testSamples) {
        String htmlFile = "<html><body>%1</body></html>";

        String innerText = "";

        double obs[] = new double[ts_obs.getNumberOfTimesteps()];
        double sim[] = new double[ts_sim.getNumberOfTimesteps()];
        double exp[] = new double[ts_sim.getNumberOfTimesteps()];

        for (int i = 0; i < ts_obs.getNumberOfTimesteps(); i++) {
    ...
    public void refresh() throws NoDataException {
        ...
        Set<String> tsSet = this.getDataSource().getDataSetNames(MeasuredTimeSerie.class);
        MeasuredTimeSerie ts[] = new MeasuredTimeSerie[tsSet.size()];
        int counter = 0;
        for (String name : tsSet) {
            ts[counter++] = (MeasuredTimeSerie) this.getDataSource().getDataSet(name);
        }
        ...
        sim = (MeasuredTimeSerie) p[0].get(0);
        int T = obs.getNumberOfTimesteps();
        ...
        if (endDate == null) {
            trainEndDate.setDate(sim.getTime(sim.getNumberOfTimesteps() / 2));
            trainEndDate.setTime(sim.getTime(sim.getNumberOfTimesteps() / 2));
        ...
        for (int i = 0; i < n; i++) {
            int row = rows.get(i);
            TimeSerie<Double> input_ts = (TimeSerie<Double>) tsTable.getModel().getValueAt(row, 1);
        ...
        TimeSeries dataset3 = new TimeSeries(obs.getName());
        ...
        TimeSerie<Double> ts_lb = null, ts_ub = null, ts_exp = null;
        ...
        DataView<Double> dslb = ViewFactory.createView(lb);
        DataView dsub = ViewFactory.createView(ub);
        DataView dsexp = ViewFactory.createView(exp);

        ts_lb = new DefaultTimeSerie<>("lb", obs.getTemporalDomain(), dslb);
        ts_ub = new DefaultTimeSerie<>("ub", obs.getTemporalDomain(), dsub);
        ts_exp = new DefaultTimeSerie<>("exp", obs.getTemporalDomain(), dsexp);
    ...
    public static void main(String[] args) {
        DefaultDataCollection dc = DefaultDataCollection.createFromFile(new File("E:/Vortraege und Paper/2013-10-17 KOS/Hymod/output/current/nsga_save2.cdat"));
