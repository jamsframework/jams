...
public class RegionalSensitivityAnalyser2 extends MCAT5Plot{
    ...
    public void refresh() throws NoDataException {
        ...
        DefaultSimpleEnsemble  param = (DefaultSimpleEnsemble)p[0].get(0);
        ArrayList<DataSet> effIn = ( ArrayList<DataSet>)p[1];

        DefaultEfficiencyEnsemble eff[] = new DefaultEfficiencyEnsemble[effIn.size()];
        for (int i=0;i<effIn.size();i++){
            eff[i] = (DefaultEfficiencyEnsemble)effIn.get(i);
        }
        ...
        for (int i=0;i<numberOfObjFct;i++){
            ...
            DefaultEfficiencyEnsemble likelihood = eff[i].CalculateLikelihood();
