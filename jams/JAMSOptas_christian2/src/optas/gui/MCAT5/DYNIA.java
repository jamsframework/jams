...
public class DYNIA extends MCAT5Plot {
    ...
    public void refresh() throws NoDataException {
        ...
        DefaultSimpleEnsemble param = (DefaultSimpleEnsemble) p[1].get(0);
        MeasuredTimeSerie obs = (MeasuredTimeSerie) p[2].get(0);

        plot.setRangeAxis(new NumberAxis(param.name));
        plot.setRangeAxis(1, new NumberAxis(obs.getName()));
        ...
        XYSeries dataset = new XYSeries(obs.getName());
        ...
        for (int i = 0; i < T; i++) {
            DefaultEfficiencyEnsemble eff = new DefaultEfficiencyEnsemble("E1(window)", n);
            ...
            DefaultEfficiencyEnsemble likelihood = eff.CalculateLikelihood();
        ...
        if (obs.getTemporalDomain().getTimeUnit() == 6){
            bg_renderer.setBlockWidth(84000*1000);
        }else if (obs.getTemporalDomain().getTimeUnit() == 2){
            bg_renderer.setBlockWidth(84000*31.00*1000);
        }else if (obs.getTemporalDomain().getTimeUnit() == 1){
            bg_renderer.setBlockWidth(84000*365.00*1000);
        }else if (obs.getTemporalDomain().getTimeUnit() == 11){
