...
public class ParetoOutputUncertainty extends MCAT5Plot {
    ...
    boolean isParetoOptimal(double eff_actual[],DefaultEfficiencyEnsemble eff_set[]){
    ...
    double[][] getMinMaxParetoTS(DefaultTimeSerieEnsemble data, DefaultEfficiencyEnsemble eff[]){
        double minMaxOptimalTS[][] = new double[2][data.getNumberOfTimesteps()];
                
        for (int i=0;i<data.getNumberOfTimesteps();i++){
        ...
        for (int i=0;i<data.getSize();i++){
            ...
            if (isParetoOptimal(actualEffSet,eff)){
                for (int t=0;t<data.getNumberOfTimesteps();t++){
    ...
    public void refresh() throws NoDataException {
        ...
        MeasuredTimeSerie obs = (MeasuredTimeSerie) p[2].get(0);


        DefaultEfficiencyEnsemble eff[] = new DefaultEfficiencyEnsemble[dataInEff.size()];
        for (int i=0;i<eff.length;i++)
            eff[i] = (DefaultEfficiencyEnsemble)dataInEff.get(i);

        plot1.setRangeAxis(new NumberAxis(ts.name));

        int time_length = ts.getNumberOfTimesteps();
        
        TimeSerie<Double> maxTS = ts.getMax();
        TimeSerie<Double> minTS = ts.getMin();
        ...
        TimeSeries observation = new TimeSeries(obs.getName());
