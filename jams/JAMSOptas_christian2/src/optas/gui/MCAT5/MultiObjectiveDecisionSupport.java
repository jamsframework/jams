...
public class MultiObjectiveDecisionSupport extends MCAT5Plot {
    ...
    DefaultTimeSerieEnsemble ts = null;
    ...
    public void refresh() throws NoDataException {
        ...
        ts = (DefaultTimeSerieEnsemble) p[0].get(0);

        MeasuredTimeSerie obs = null;
        if (p[2].size() > 0) {
            obs = (MeasuredTimeSerie) p[2].get(0);
        }
    ...
    private void initDataSet() {
        Set<String> xSet = this.getDataSource().getDataSetNames(Parameter.class);
        x = new SimpleEnsemble[xSet.size()];
        int counter = 0;
        for (String name : xSet) {
            x[counter++] = this.getDataSource().getSimpleEnsemble(name);
        }
        Set<String> ySet = this.getDataSource().getDataSetNames(Efficiency.class);
        y = new DefaultSimpleEnsemble[ySet.size()];
        counter = 0;
        ...
        for (int sampleNumber = 0; sampleNumber < candidates.size(); ++sampleNumber) {
            ...
            for (int i = 0; i < sample.F().length; i++) {
                if (visibleCandidates[sampleNumber]) {
                    ...
                    categoryDataset.addValue(neff, "Sample " + sampleNumber, y[i].getName());
                } else {
                    categoryDataset.addValue(Double.NaN, "Sample " + sampleNumber, y[i].getName());
                }
            }
        }
    ...
    public static void main(String[] args) {
        DefaultDataCollection dc = DefaultDataCollection.createFromFile(new File("E:\\ModelData\\Testgebiete\\J2000\\Gehlberg\\output\\20130824_003244\\2013_08_21_sa_small2.cdat"));
