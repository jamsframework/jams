...
public class BaseFlow {
    ...
    public static double[] groundwaterWindowMethod(TimeSerie<Double> hydrograph) {
        return groundwaterWindowMethod(hydrograph, 150);
    }
    public static double[] groundwaterWindowMethod(TimeSerie<Double> hydrograph, int winSize) {
        int t = (int) hydrograph.getTemporalDomain().getNumberOfTimesteps();
...
//default 150
    ...
    public static TimeSerie<Double> calculateGroundwater(TimeSerie hydrograph, int windowSize) {
        try {
            DataView groundwaterData = ViewFactory.createView(groundwaterWindowMethod(hydrograph, windowSize));
            TimeSerie t = new DefaultTimeSerie("groundwater", hydrograph.getTemporalDomain(), groundwaterData);
            return t;
        } catch (OPTASException e) {
            System.out.println(e);
        }
        return null;
    }

    public static ArrayList<HydrographSection> calculateBaseFlowPeriods(TimeSerie<Double> hydrograph, double threshold) {
        return calculateBaseFlowPeriods(hydrograph, threshold, 150);
    }
    public static ArrayList<HydrographSection> calculateBaseFlowPeriods(TimeSerie<Double> hydrograph, double threshold, int windowSize) {
        ArrayList<HydrographSection> list = new ArrayList<HydrographSection>();
        int n = (int) hydrograph.getTemporalDomain().getNumberOfTimesteps();
        TimeSerie<Double> groundwater = calculateGroundwater(hydrograph, windowSize);
        int i = 0;
        while (i < n) {
