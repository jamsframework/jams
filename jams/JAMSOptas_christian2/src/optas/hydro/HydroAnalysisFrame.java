...
public class HydroAnalysisFrame extends JFrame {
    ...
    DefaultDataCollection ensemble;
...
//        scheme[index].calcOptimizationScheme();
    ...
    public HydroAnalysisFrame() {
        ...
        openObservation.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (result == JFileChooser.APPROVE_OPTION) {
                    try {
                        J2KTimeSerieReader reader = new J2KTimeSerieReader(j2kFile_chooser.getSelectedFile());;
                        String[] attr = reader.getData().getAttributeNames();
                        SelectionDialog selectionDialog = new SelectionDialog(null, new TreeSet<Object>(Arrays.asList(attr)));

                        String selectedName = (String) selectionDialog.getSelection();
                        int index = 0;
                        for (index = 0; index < attr.length; index++) {
                            if (attr[index].equals(selectedName)) {
                                break;
                            }
                        }
                        chart.setHydrograph(reader.getData().getColumn(index));
                    } catch (IOException ioe) {
                        Logger.getLogger(HydroAnalysisFrame.this.getClass().getName()).log(Level.SEVERE, ioe.toString(), ioe);
                    }
                }
            }
        });
        ...
        openEnsemble.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (result == JFileChooser.APPROVE_OPTION) {
                    HydroAnalysisFrame.this.ensemble = DefaultDataCollection.createFromFile(chooser.getSelectedFile());
                    Set<String> obsDatasets = ensemble.getDataSetNames(MeasuredTimeSerie.class);
                    Set<String> paramDatasets = ensemble.getDataSetNames(Parameter.class);
                    Set<String> effDatasets = ensemble.getDataSetNames(Efficiency.class);
                    Set<String> simDatasets = ensemble.getDataSetNames(TimeSerie.class);
                    ...
                    if (!obsDatasets.isEmpty()) {
                        MeasuredTimeSerie measurement = (MeasuredTimeSerie) HydroAnalysisFrame.this.ensemble.getDataSet(obsDatasets.iterator().next());
    ...
    private void calcWeights() {
        ...
        DefaultTimeSerieEnsemble tsEnsemble = HydroAnalysisFrame.this.ensemble.getTimeserieEnsemble(simItem);
        ...
        obsDatasets.addItemListener(new ItemListener() {
            ...
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == e.SELECTED) {
                    MeasuredTimeSerie measurement = (MeasuredTimeSerie) HydroAnalysisFrame.this.ensemble.getDataSet((String) e.getItem());
                    chart.setHydrograph(measurement);
                }
            }
        });
