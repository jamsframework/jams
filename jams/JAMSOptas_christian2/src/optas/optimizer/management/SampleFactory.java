...
import optas.data.ensemble.ParameterEnsemble;
...
public class SampleFactory implements Serializable {
    ...
    private DataView<Integer> generateIds() {
        DataView<Integer> ids = new AbstractListView<Integer, ArrayList<Sample>>(getSampleList()) {
    ...
    public ParameterEnsemble[] getParameterEnsemble() {
        ParameterEnsemble x[] = new ParameterEnsemble[this.stats.n()];

        for (int i = 0; i < x.length; i++) {
            DataView<Double> values = new AbstractListView<Double, ArrayList<Sample>>(getSampleList(), i) {
                final int column;
                ...
                public Double getValue(int i) {
                    return input.get(i).x[column];
                }

            };

            x[i] = new ParameterEnsemble(
                    new DefaultSimpleEnsemble("param_" + i, null, generateIds(), values));
        }
        return x;
    }

    public NegativeEfficiencyEnsemble[] getObjectivesEnsemble() {
        NegativeEfficiencyEnsemble y[] = new NegativeEfficiencyEnsemble[this.stats.m()];
        ...
        for (int i = 0; i < y.length; i++) {
            DataView<Double> values = new AbstractListView<Double, ArrayList<Sample>>(getSampleList(), i) {
                ...
                @Override
                public Double getValue(int i) {
                    return input.get(i).fx[column];
                }

            };

            y[i] = new NegativeEfficiencyEnsemble("obj_" + i, null, generateIds(), values);
        }
        return y;
    }
