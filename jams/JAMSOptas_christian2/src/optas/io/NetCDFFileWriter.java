...
public class NetCDFFileWriter {
    ...
    public void write(DataCollection collection) throws IOException, UnsupportedOperationException, OPTASException {
        ...
        Set<String> dataSetNames = collection.getDataSetNames();
        for (String name : dataSetNames) {
            ...
            } else if (dataSet instanceof MeasuredTimeSerie) {
                dimensions.add(time);
            } else if (dataSet instanceof DefaultSimpleEnsemble || dataSet instanceof DefaultEfficiencyEnsemble) {
                dimensions.add(modelRunID);
            } else {
            ...
            if (dimensions.contains(time)) {
                ...
                if (dataSet instanceof DefaultTimeSerieEnsemble) {
                    interval = ((DefaultTimeSerieEnsemble) dataSet).getTemporalDomain();
                }
                if (dataSet instanceof MeasuredTimeSerie) {
                    interval = ((MeasuredTimeSerie) dataSet).getTemporalDomain();
                }
            ...
            if (dataSet instanceof DefaultEfficiencyEnsemble) {
                var.addAttribute(new Attribute("isPositiveBest", String.valueOf(((DefaultEfficiencyEnsemble) dataSet).isPositiveBest())));
            }
        }
        ...
        for (String name : dataSetNames) {
            ...
            if (dataSet instanceof DefaultTimeSerieEnsemble) {
                ...
                DefaultTimeSerieEnsemble ensemble = (DefaultTimeSerieEnsemble) dataSet;
                TimeInterval dataSetInterval = ensemble.getTemporalDomain();
                Integer[] dataSetModelRuns = ensemble.getIds();
                
                if (!hasSuitableTimeInterval(collectionInterval, dataSetInterval)) {
                    throw new OPTASException("Time intervals don't match.");
                } else if (!hasSuitableModelRuns(collectionModelRuns, dataSetModelRuns)) {
                    throw new OPTASException("Model run IDs don't match.");
                } else {
                ...
                MeasuredTimeSerie measurement = (MeasuredTimeSerie) dataSet;
                TimeInterval dataSetInterval = measurement.getTemporalDomain();
                ...
                DefaultSimpleEnsemble ensemble = (DefaultSimpleEnsemble) dataSet;
                Integer[] dataSetModelRuns = ensemble.getIds();
