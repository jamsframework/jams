...
public class ImportMonteCarloData implements Serializable {
    ...
    private void init() {
        ...
        simpleDatasetClasses.put(EnsembleType.Measurement, MeasuredTimeSerie.class);
        ...
        timeSerieDatasetClasses.put(EnsembleType.Measurement, MeasuredTimeSerie.class);
    ...
    private DataCollection buildEnsemble() throws ImportMonteCarloException {
        ensemble = DataCollection.getInstance("Mein Ensemble");
                ...
                if (simpleDatasetClasses.containsKey(dataSetClassName) && p instanceof SimpleSerieProcessor) {
                    if (p instanceof SimpleSerieProcessor) {
                        ...
                        if (s.isTimeSerie()){
                            ...
                            DataView<Double> data = ViewFactory.createView(values);                            
                            TimeSerie ts = new DefaultTimeSerie(a.getName(), ensembleTime, data );
                            
                            ensemble.addGlobalDataSet(MeasuredTimeSerie.getInstance(ts));
                        } else {
                            for (String id : ids) {
                                Integer intID = null;
                                try {
                                ...
                                DefaultModelRun r = new DefaultModelRun("Run # " + intID, intID);
                                ...
                                SimpleDataSet nonTypedSDS = new SimpleDataSet(a.getName(), m.get(row, 0));
                                row++;
            ...
            try {
                if (timeSerieDatasetClasses.containsKey(dataSetClassName) && p instanceof EnsembleTimeSeriesProcessor) {
                    ...
                    if (this.timeSerieDatasetClasses.get(dataSetClassName).equals(TimeSerie.class)) {
                        int col = 0;
                        for (Long id : ids) {
                            DefaultModelRun r = new DefaultModelRun("Run #" + id.intValue(), id.intValue());
                            DataView<Double> data = ViewFactory.createView(m.getCol(col));                            
                            r.addDataSet(new DefaultTimeSerie(a.getName(), ensembleTime, data ));
                            
                            col++;
                            ensemble.addModelRun(r);
                        }
                    } else if (this.timeSerieDatasetClasses.get(dataSetClassName).equals(MeasuredTimeSerie.class)) {
                        int col = 0;
                        MeasuredTimeSerie ts = null;
                        for (Long id : ids) {
                            DefaultModelRun r = new DefaultModelRun("Run #" + id.intValue(), id.intValue());
                            DataView<Double> data = ViewFactory.createView(m.getCol(col));
                            MeasuredTimeSerie ts2 = MeasuredTimeSerie.getInstance(new DefaultTimeSerie(a.getName(), ensembleTime, data ));
                            if (ts == null) {
                                ts = ts2;
                            } else {
                                for (int i = 0; i < ts.getTemporalDomain().getNumberOfTimesteps(); i++) {
                                    if (ts.getValue(i) != ts2.getValue(i)) {
                                        throw new OPTASException(JAMS.i18n("timeserie_ensemble_could_not_be_used_as_measurement"));
                                    }
                                }
                            }
                            col++;
                        }
                        ensemble.addGlobalDataSet(ts);
                    }
                }
            } catch (SQLException sqle) {
                sqle.printStackTrace();
    ...
    public void setEnsemble(DefaultDataCollection dc) {
        this.ensemble = dc;
    }
