...
public class SobolsMethodTemporal extends TemporalSensitivityAnalysis {
    ...
    DefaultTimeSerieEnsemble tsStar;
    ...
    public SobolsMethodTemporal(SimpleEnsemble parameter[], DefaultEfficiencyEnsemble o, DefaultTimeSerieEnsemble ts, MeasuredTimeSerie obs) {
    ...
    protected void sampleData(int size) {
        ...
        SampleFactory f = sampler.getFactory();
        x = f.getParameterEnsemble();

        Preconditions.checkArgument(x.length > 0, "There must be at least one parameter to sample!");
        DataView<TimeSerie<Double>> values = new AbstractListView<TimeSerie<Double>, NegativeEfficiencyEnsemble[]>(f.getObjectivesEnsemble(), ts.getTemporalDomain()) {

            TimeInterval temporalDomain;

            {
                temporalDomain = (TimeInterval) args[0];
            }
            ...
            public TimeSerie<Double> getValue(int i) {
                DataView values = input[i].values();
                return new DefaultTimeSerie<>("ts_" + i, null, temporalDomain, values);
            }

        };

        tsStar = new DefaultTimeSerieEnsemble("tsStar", null, x[0].ids(), values);
        L = tsStar.getSize();
    }
    ...
    private void updateData() {
        ...
        for (int i = 0; i < Lh; i++) {
            ...
            yA[i] = ArrayUtils.toPrimitive(ViewFactory.toArray(this.tsStar.getValue(id_iA).values()));
            B[i] = transformToUnitCube(x0B);
            yB[i] = ArrayUtils.toPrimitive(ViewFactory.toArray(this.tsStar.getValue(id_iB).values()));
    ...
    private void calcSensitivity() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            set.add(i);
            log("Calculating Sensitivity for " + x[i].getName());
