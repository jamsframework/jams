...
Most dependencies are bundled in `libs-repo/` (see above), but a few
libraries — notably GeoTools — are pulled from Maven Central on first
build. If your network requires an HTTP/HTTPS proxy, this download can
fail even though `http_proxy` / `https_proxy` environment variables are
set: Maven does **not** read those automatically — the proxy has to be
configured in `~/.m2/settings.xml` instead.
