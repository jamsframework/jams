...
public class MapLegend extends JPanel {
	...
	public static Map coloring (Set<Double> d, int numRanges, String rangeColor) throws Exception {
		...
		if (numRanges > d.size()) {numRanges = d.size();}
		...
		if (rangeColor.equalsIgnoreCase("green")) {
			int rangecounts = 1;
			for (int i = range; i<=510; i+=range) {
				if (rangecounts == numRanges) {
					...
					else colors.put(new Color(i-255,255,i-255), (Double)colmap.get(i));
				}
				rangecounts++;
			}
		} else
		if (rangeColor.equalsIgnoreCase("blue")) {
			int rangecounts = 1;
			for (int i = range; i<=510; i+=range) {
				if (rangecounts == numRanges) {
					...
					else colors.put(new Color(i-255,i-255,255), (Double)colmap.get(i));
				}
				rangecounts++;
			}
		}
		else System.out.println("Not supported color for rangeColor");
