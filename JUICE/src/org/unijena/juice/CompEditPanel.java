...
public class CompEditPanel extends JPanel {
    ...
    private VarEditDlg varEditDlg;
    private AttributeEditDlg attrEditDlg;
    private ModelView view;
    private JAMSNode node;
    private JTabbedPane tabPane;
    
    public CompEditPanel(ModelView view) {
        super();
        this.view = view;
        init();
    }
    ...
    private void init() {
        ...
        varRowSM.addListSelectionListener(new ListSelectionListener() {
            public void valueChanged(ListSelectionEvent e) {
                ...
                if (!lsm.isSelectionEmpty()) {
                    CompEditPanel.this.selectedVarRow = lsm.getMinSelectionIndex();
                    CompEditPanel.this.varEditButton.setEnabled(true);
                    CompEditPanel.this.varResetButton.setEnabled(true);
                } else {
                    CompEditPanel.this.selectedVarRow = -1;
                    CompEditPanel.this.varEditButton.setEnabled(false);
                    CompEditPanel.this.varResetButton.setEnabled(false);
                }
            }
        });
        ...
        attrRowSM.addListSelectionListener(new ListSelectionListener() {
            public void valueChanged(ListSelectionEvent e) {
                ...
                ListSelectionModel lsm = (ListSelectionModel)e.getSource();
                if (!lsm.isSelectionEmpty()) {
                    CompEditPanel.this.selectedAttrRow = lsm.getMinSelectionIndex();
                    CompEditPanel.this.attributeEditButton.setEnabled(true);
                    CompEditPanel.this.attributeDeleteButton.setEnabled(true);
                } else {
                    CompEditPanel.this.selectedAttrRow = -1;
                    CompEditPanel.this.attributeEditButton.setEnabled(false);
                    CompEditPanel.this.attributeDeleteButton.setEnabled(false);
                }
            }
        });
        ...
        tabPane.add("Attributes", attributePanel);
        tabPane.setEnabledAt(1, false);
        
        LHelper.addGBComponent(componentPanel, mainLayout, new JPanel(), 0, 2, 4, 1, 1.0, 1.0);
        LHelper.addGBComponent(componentPanel, mainLayout, new JLabel("Data:"), 0, 10, 4, 1, 0, 0);
    ...
    private void showAttributeEditDlg() {
        ...
        if (attrEditDlg == null) {
            attrEditDlg = new AttributeEditDlg(JUICE.getJuiceFrame());
        }
    ...
    private void showVarEditDlg() {
        ...
        //create the dialog if it not yet existing
        if (varEditDlg == null) {
            varEditDlg = new VarEditDlg(JUICE.getJuiceFrame(), view);
        }
        ...
        if (varEditDlg.getResult() == VarEditDlg.APPROVE_OPTION) {
    ...
    private void updateVars() {
        ...
        for (String name : varNameList) {
            ...
            rowData.add(var.type.getSimpleName());
            String accessType = "";
