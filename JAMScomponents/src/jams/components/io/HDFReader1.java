...
public class HDFReader1 extends JAMSComponent {
    ...
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READ,
            description = "MODIS cell IDs"
    )
    public Attribute.DoubleArray cellIDs;
...
//        double rowArray[] = rows.getValue();
        double cellIDArray[] = cellIDs.getValue();
...
//            v += wArray[i] * getValue((int) colArray[i], (int) rowArray[i]) * 0.1;
            v += wArray[i] * getValue((int) cellIDArray[i]) * 0.1;
        }
        value.setValue(v);
    }

    public static void main(String[] args) {
        HDFReader1 reader = new HDFReader1();
    ...
    private double getValue(int cellID) {
        if (actData == null) {
            return jams.JAMS.getMissingDataValue();
        }
        double v = actData[cellID];
