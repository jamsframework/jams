...
public class DYNIA extends MCAT5Plot {
    ...
    public void refresh() {
            ...
            for (int k=0;k<n;k++){
                ...
                for (int j=i-window_size;j<=i+window_size;j++){
                    if (j < 0 || j >= T)
                        ...
                        cur_sim[counter++] = ts.get(j, k);
                    }
                }
                eff.add(new Integer(k), Efficiencies.CalculateE(cur_obs,cur_sim,2));
            }
        ...
        if (plot.getRangeAxis() != null) plot.getRangeAxis(0).setRange(new Range(minParam,maxParam));
        if (plot.getDomainAxis() != null)plot.getDomainAxis().setRange(new Range(obs.getTime(0).getTime(),obs.getTime(T-1).getTime()));
