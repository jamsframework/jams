 ...
 * @author S. Kralisch
 */
public class IntegerAccessor implements JAMSEntityDataAccessor {
    
    JAMSInteger componentObject;
    JAMSInteger[] entityObject;
    int index;
    int accessType;
    
    public IntegerAccessor(JAMSEntity[] entities, JAMSData dataObject, String attributeName, int accessType) throws JAMSEntity.NoSuchAttributeException {
        ...
        for (int i = 0; i < entities.length; i++) {
            if (entities[i].existsAttribute(attributeName)) {
                ...
                if (accessType != JAMSEntityDataAccessor.READ_ACCESS) {
                    entityObject[i] = new JAMSInteger();
