 ...
 */
public class KGEprimeAllPar extends EfficienciesCalculator {

    double pow = 2.0;

    public KGEprimeAllPar() {
    ...
    public double[] calc(double m[], double s[]) {
        ...
        return new double[] {1 - Math.sqrt(Math.pow(gamma - 1, pow) + Math.pow(beta - 1, pow) + Math.pow(r - 1, pow)), r, gamma, beta};
    }

    @Override
    public double calcNormative(double t1[], double t2[]) {
        return 1.0 - calc(t1, t2)[0];
    }
}
